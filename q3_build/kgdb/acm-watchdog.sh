...
#
# Twice during development a UDC unbind left the headset with no USB at
# all, needing a physical power cycle. A single timed check was not enough:
# if the one rebind attempt failed, nothing retried. So this retries for
...
while [ $i -lt 45 ]; do
  ...
  echo "$U" > $G/UDC 2>/dev/null
  [ -n "$(cat $G/UDC 2>/dev/null)" ] && { echo "rebound WITHOUT kgdb function"; start vendor.usb_default; exit 0; }
  i=$((i+1))
done
echo "watchdog giving up; starting vendor.usb_default"
