...
# --- 2. Kernel source -----------------------------------------------------
KERNEL_COMMIT="2ec81fa7af4875987bb74ba132661dfb4ade999a"
if [ ! -d "$KERNEL_DIR" ]; then
  log "Cloning oculus-linux-kernel at commit $KERNEL_COMMIT"
...
if ! git diff --quiet -- . 2>/dev/null || [ -n "$(git status --short --untracked-files=no)" ]; then
  ...
  elif git apply --check --3way "$WORK_DIR/oculus-kernel-fixes.patch" 2>/dev/null; then
    echo "Patch needs --3way (source tree has drifted from the commit it was made against); applying with --3way."
    git apply --3way "$WORK_DIR/oculus-kernel-fixes.patch"
  else
    # Tree has drifted from the commit the patch was authored against. Find
    # exactly which file(s) no longer match, exclude just those, and apply
    # the rest -- a hunk that no longer finds its context usually means the
    # code it targeted was itself restructured/removed upstream, so the
    # underlying issue may no longer apply in that form.
    FAILED_FILES="$(git apply --check "$WORK_DIR/oculus-kernel-fixes.patch" 2>&1 \
      | sed -n 's/^error: patch failed: \(.*\):[0-9]*$/\1/p' | sort -u || true)"
    if [ -z "$FAILED_FILES" ]; then
      echo "ERROR: oculus-kernel-fixes.patch does not apply cleanly to this commit, and the failing file(s) could not be identified automatically." >&2
      exit 1
    fi
    echo "Patch hunk(s) failed for the following file(s) (tree has drifted from the commit the patch was authored against):" >&2
    echo "$FAILED_FILES" | sed 's/^/  - /' >&2
    EXCLUDE_ARGS=()
    for f in $FAILED_FILES; do EXCLUDE_ARGS+=(--exclude="$f"); done
    if git apply --check "${EXCLUDE_ARGS[@]}" "$WORK_DIR/oculus-kernel-fixes.patch" 2>/dev/null; then
      echo "Applying patch excluding the above file(s); proceeding without those fixes." >&2
      git apply "${EXCLUDE_ARGS[@]}" "$WORK_DIR/oculus-kernel-fixes.patch"
    else
      echo "ERROR: oculus-kernel-fixes.patch still does not apply even excluding the above file(s); manual conflict resolution is needed." >&2
      exit 1
    fi
  fi
fi
