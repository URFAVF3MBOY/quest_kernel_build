#!/bin/bash
# Reproduces: cross-compiling oculus-linux-kernel (oculus-quest2-kernel-master)
# for real Quest 2 hardware from a supplied kernel.config, plus a QEMU-bootable
...
# toolchain vintage as the Quest 3 kernel, even though this is a much older
# Linux 4.19 tree). AOSP's gitiles instance prunes old clang versions off the
# tip of its "master" branch as new ones ship, so the exact commit that still
# has this version checked in has to be found via one of its long-lived
...
# here. arch/arm64/Kconfig selects COMMON_CLK unconditionally (no
# "if !ARCH_QCOM" exclusion, confirmed via CONFIG_COMMON_CLK=y already in the
# real-device kernel.config), so QEMU's generic virt-board clock/AMBA devices
# bind normally and QEMU's own auto-generated DTB is used (no -dtb flag).
...
cat > "$BUILDROOT_DIR/configs/oculus_quest2_qemu_defconfig" << 'EOF'
# Architecture
BR2_aarch64=y
BR2_cortex_a53=y
...
# Linux headers: matches this kernel's own version (4.19).
...
cd "$BUILDROOT_DIR"
make O=output oculus_quest2_qemu_defconfig
...
PATH="$CLEAN_PATH" HOSTCC=gcc-12 HOSTCXX=g++-12 make O=output -j"$(nproc)"
...
#!/bin/bash
# Boot the QEMU-bootable Oculus Quest 2 kernel variant with the Buildroot
...
set -e
cd "\$(dirname "\$0")"
exec qemu-system-aarch64 -M virt -cpu cortex-a76 -m 6144 -nographic -smp 8 \\
