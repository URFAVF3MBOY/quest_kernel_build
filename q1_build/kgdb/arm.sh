#!/bin/bash
# Arm kgdb-over-USB on a Quest 1 already running a KGDB=1 kernel.
#
...
#
# Assumes a plain Linux host with the headset already visible to adb and its
# CDC-ACM port appearing as /dev/ttyACM*. Composing the gadget bounces the
# UDC, so the device re-enumerates several times and adb briefly goes away;
# this just waits it out.
set -u
...
status() {
  ...
  # 4.4.205-perf+ - so print the build number and date too.
  echo "kernel:    $(timeout 20 "$ADB" shell 'uname -r; uname -v' 2>&1 | tr '\n' ' ')"
...
if ! timeout 20 "$ADB" shell 'ls /sys/module/u_serial/parameters/kgdb_port' >/dev/null 2>&1; then
  ...
  echo "This kernel was not built with KGDB=1. uname: $(timeout 20 "$ADB" shell uname -a 2>&1)" >&2
  exit 1
fi
...
if [ ! -e "$TTY" ]; then
  ...
  timeout 30 "$ADB" root >/dev/null 2>&1; sleep 4
else
  log "acm.kgdb already composed on $TTY"
fi
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  [ -e "$TTY" ] && stty -F "$TTY" raw -echo 115200 2>/dev/null
  rm -f /tmp/kgdb-selftest.bin
  ( timeout 12 cat "$TTY" > /tmp/kgdb-selftest.bin 2>/dev/null & )
  sleep 2
...
Unless the kernel was built with KGDB_DISABLE_WDT=1, keep commands short:
the watchdogs reset the headset after roughly 10-30s with the CPUs stopped.
See README.md.
EOF
